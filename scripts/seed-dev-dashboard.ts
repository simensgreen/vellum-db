...
import { measureLiveSnapshot, type StatsSnapshot } from "../src/core/stats-store.ts"
...
function snapshotForDay(dayIndex: number, target: StatsSnapshot): StatsSnapshot {
    const progress = dayIndex / HISTORY_DAYS
    const wave = Math.sin(dayIndex * 0.45)
    ...
    const databaseBytes = Math.max(
        bytesFloor,
        Math.floor(bytesFloor + (target.database_bytes - bytesFloor) * progress + wave * 512)
    )
...
function seedHistoricalStats(today: number, target: StatsSnapshot): void {
    ...
    for (let dayIndex = 0; dayIndex <= HISTORY_DAYS; dayIndex += 1) {
        ...
        const snapshot = dayIndex === HISTORY_DAYS ? target : snapshotForDay(dayIndex, target)
        database
            .query(
...
function seedTypeShowcaseIfEmpty(): void {
    ...
    const database = getDatabase()
    const firstEvent = database.query("SELECT event_id FROM events LIMIT 1").get() as {
        event_id: string
    } | null
...
function seedWorkTables(): void {
    ...
    if (tableRowCount("projects") === 0) {
        ...
        const regionByName = new Map(regions.map((region) => [region.name, region.region_id]))
    ...
    if (tableRowCount("tasks") === 0) {
        ...
        const projectByName = new Map(projects.map((project) => [project.name, project.project_id]))
