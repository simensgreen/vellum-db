...
import { POST as createTablePost, GET as listTablesGet } from "../routes/tables.ts"
...
async function dispatchApi(request: Request): Promise<Response | null> {
    ...
    if (routePath === "/rows/commit" && request.method === "POST" && response.ok) {
    ...
    if (routePath === "/import" && request.method === "POST" && response.ok) {
        const tableName = url.searchParams.get("table")
        if (tableName) {
            broadcastSync([tableDataTag(tableName)])
        }
    ...
    if (routePath === "/tables" && request.method === "POST" && response.ok) {
        broadcastSync(["vellum-db:tables"])
    }

    if (routePath === "/tables/alter" && request.method === "POST" && response.ok) {
        const tableName = url.searchParams.get("table")
        broadcastSync(
            tableName ? ["vellum-db:tables", tableDataTag(tableName)] : ["vellum-db:tables"]
        )
    }
...
function openDevDatabase(): string {
    ...
    openDatabase(dir, parseConfig({ maxRowsPerQuery: 500, rawSqlMode: "select-only" }))
    ensureMetaSchema()
