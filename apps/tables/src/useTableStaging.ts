...
import type { RowCommitBody } from "./api.ts";
import {
...
export function useTableStaging(
  ...
  const [insertFocusTarget, setInsertFocusTarget] =
    useState<InsertFocusTarget | null>(null);
  ...
  const phantomDirty = useMemo(
    () => draftDiffersFromDefault(phantom, table),
    [phantom, table],
  );
  ...
  const pendingCount = useMemo(
    () => countChanges(patch, phantomDirty),
    [patch, phantomDirty],
  );
  ...
  const setCell = useCallback(
    (
      ...
      setPatch((current) =>
        setCellUpdate(current, pk, slug, raw, original, columns),
      );
    },
    [columns],
  );
  ...
  const updateInsert = useCallback(
    (localId: string, slug: string, raw: string) => {
