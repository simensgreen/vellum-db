...
export function DatabaseApp() {
    ...
    async (
      ...
      try {
        const response = await runView(
          viewSlug,
          paramsFromForm(paramNames, values),
        );
        setViewRunState(response);
      } catch (error) {
        setViewRunError(
          error instanceof Error ? error.message : "Failed to run view",
        );
      } finally {
  ...
  useEffect(() => {
    ...
    if (selectedViewSummary.param_names.length === 0) {
      void executeViewRun(
        selectedViewSummary.slug,
        selectedViewSummary.param_names,
        {},
      );
    }
  ...
  useEffect(() => {
    const unsubscribeTables = vellumSubscribe(
      { tags: subscribeTagsForTableList() },
      () => {
        void loadTables();
      },
    );
    const unsubscribeViews = vellumSubscribe(
      { tags: subscribeTagsForViewList() },
      () => {
        void loadViews();
      },
    );
  ...
  const overviewSelected =
    mainView === "rows" && selectedTable === null && selectedView === null;
