...
export function RowsGrid({
  ...
  const allVisibleSelected =
    ...
    visibleRowIds.every((rowId) => staging.isMarkedDelete(rowId));
  const someVisibleSelected = visibleRowIds.some((rowId) =>
    staging.isMarkedDelete(rowId),
  );
  ...
  async function handleImportFile(event: Event): Promise<void> {
    ...
    try {
      ...
      showToastError(
        error instanceof Error ? error.message : "Failed to import table",
      );
    } finally {
      setIoBusy(false);
    }
                ...
                <tr key={rowId} class={rowClass}>
                  <td class="rows-grid__select-col">
                    <RowActionCell
                      ...
                      onMarkForDelete={() => staging.toggleDelete(rowId, true)}
                      onUnmarkDelete={() => staging.toggleDelete(rowId, false)}
                    />
                  </td>
                  <RowCells
                    table={table}
