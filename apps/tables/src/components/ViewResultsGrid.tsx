...
function formatRowRange(offset: number, limit: number, count: number, totalCount?: number): string {
    if (count === 0) {
        return "0 rows"
    }
    const rangeEnd = Math.min(offset + limit, count + offset)
    const totalLabel = totalCount !== undefined ? formatCount(totalCount) : formatCount(count)
...
export function ViewResultsGrid({
    ...
    const columns = rows && rows.length > 0 ? Object.keys(rows[0] ?? {}) : rows !== null ? [] : []
