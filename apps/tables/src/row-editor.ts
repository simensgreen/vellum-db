import type {
  ColumnData,
  ColumnDefinition,
} from "vellum-db/core/table/types";
...
export function rowIdFromRecord(
...
export function visibleColumns(table: TableSummary): ColumnDefinition[] {
  return table.definition.columns.filter(
    (column) => column.uiMode !== "hidden",
  );
}
...
export function placeholderForColumn(column: ColumnDefinition): string {
  switch (column.data.type) {
    case "int":
      return column.data.min !== undefined
        ? String(column.data.min)
        : "0";
    case "float":
      return "0.0";
    case "bool":
...
export function draftDiffersFromDefault(
  draft: RowDraft,
  table: TableSummary,
): boolean {
...
function constraintValidationError(
  column: ColumnDefinition,
  parsed: unknown,
): string | null {
  const data = column.data;
  switch (data.type) {
    case "int":
      ...
      if (data.min !== undefined && parsed < data.min) {
        return (
          ...
          `Number must be at least ${data.min}`
        );
      }
      if (data.max !== undefined && parsed > data.max) {
        return (
          customValidationError(data) ??
          `Number must be at most ${data.max}`
        );
      }
      return null;
    case "float":
    ...
    case "str":
      ...
      if (data.minLen !== undefined && parsed.length < data.minLen) {
        return (
          customValidationError(data) ??
          `Text must be at least ${data.minLen} characters`
        );
      }
      if (data.maxLen !== undefined && parsed.length > data.maxLen) {
        return (
          customValidationError(data) ??
          `Text must be at most ${data.maxLen} characters`
        );
      }
      if (data.regex?.regex) {
    ...
    case "enum":
      if (typeof parsed !== "string" || !data.variants.includes(parsed)) {
        return (
          customValidationError(data) ??
          `Select one of: ${data.variants.join(", ")}`
        );
      }
      return null;
    case "timestamp":
