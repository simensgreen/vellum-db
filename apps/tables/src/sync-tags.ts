...
export function tableNameFromDataTag(tag: string): string | null {
  return tag.startsWith(TABLE_DATA_PREFIX)
    ? tag.slice(TABLE_DATA_PREFIX.length)
    : null;
}

/** Tag batches for publish (core) */
export function invalidationTagsForCatalogChange(
  tableName?: string,
): readonly string[] {
  return tableName
    ? [SYNC_TAGS.tables, tableDataTag(tableName)]
    : [SYNC_TAGS.tables];
}

export function invalidationTagsForRowMutation(
  tableName: string,
): readonly string[] {
