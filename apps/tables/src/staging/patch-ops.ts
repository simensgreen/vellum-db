...
function valuesEqualForColumn(column: ColumnDefinition, left: unknown, right: unknown): boolean {
    if (left === right) {
        return true
    }
...
export function updateInsertCell(
    ...
    next.insert = next.insert.map((entry) =>
        entry.localId === localId ? { ...entry, cells: { ...entry.cells, [slug]: raw } } : entry
    )
    return next
}

export function removeInsert(patch: StagingPatch, localId: string): StagingPatch {
    const next = clonePatch(patch)
    next.insert = next.insert.filter((entry) => entry.localId !== localId)
    return next
}

export function toggleDelete(patch: StagingPatch, pk: string, marked: boolean): StagingPatch {
    const next = clonePatch(patch)
...
export function countChanges(patch: StagingPatch, phantomDirty: boolean): number {
