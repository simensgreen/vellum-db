...
import { deleteView, listViews, saveView, substituteParams } from "../src/core/views.ts"
import {
    closeDatabase,
    getDatabase,
    getDatabasePath,
...
describe("vellum-db core", () => {
    ...
    test("scope filter and list pagination", () => {
        ...
        try {
            ...
            expect(listTables({ scope: "project_a" }).tables.map((table) => table.name)).toEqual([
                "alpha_tasks"
            ])
    ...
    test("db_dump and db_load round-trip json csv jsonl xlsx", () => {
        ...
        try {
            ...
            const itemsTable = listTables().tables.find((entry) => entry.name === "items")!
            for (const row of [
            ...
            expect(dumpTableToFile({ table: "items", path: jsonPath, mode: "json" }).count).toBe(2)
            ...
            expect(
                loadTableFromFile({ table: "items", path: jsonPath, mode: "json" }).inserted
            ).toBe(2)

            const csvPath = join(dir, "items.csv")
            dumpTableToFile({ table: "items", path: csvPath, mode: "csv" })
            getDatabase().run(`DELETE FROM "items"`)
            expect(loadTableFromFile({ table: "items", path: csvPath, mode: "csv" }).inserted).toBe(
                2
            )
            ...
            getDatabase().run(`DELETE FROM "items"`)
            expect(
                loadTableFromFile({ table: "items", path: jsonlPath, mode: "jsonl" }).inserted
            ).toBe(2)
            ...
            getDatabase().run(`DELETE FROM "items"`)
            expect(
                loadTableFromFile({ table: "items", path: xlsxPath, mode: "xlsx" }).inserted
            ).toBe(2)
    ...
    test("ioModeFromFilename maps supported extensions", () => {
        ...
        expect(() => ioModeFromFilename("no-extension")).toThrow(/Cannot detect format/)
    ...
    test("resolveWorkspaceDir prefers VELLUM_WORKSPACE_DIR", () => {
        ...
        try {
            ...
            expect(resolveWorkspaceDir("/anything/plugins/vellum-db/data")).toBe(
                "/tmp/vellum-workspace-fixture"
            )
        } finally {
