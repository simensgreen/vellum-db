...
import { optionalJsonQueryString, requiredJsonQueryString } from "./json-query.ts"
...
const NonEmptyRowPatchSchema = RowRecordSchema.refine((patch) => Object.keys(patch).length > 0, {
...
export const RowCommitBodySchema = z
    .object({
        ...
        update: z.record(z.string().min(1), NonEmptyRowPatchSchema).default({}),
        delete: z.array(z.string().min(1)).default([])
    })
    .refine(
        (body) => body.insert.length + Object.keys(body.update).length + body.delete.length > 0,
        {
