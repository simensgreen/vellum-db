...
export function validateAgainstSchema(schema: object, value: unknown, label: string): void {
    const validator = ajv.compile(schema)
    if (!validator(value)) {
        throw new Error(`${label} validation failed: ${formatAjvErrors(validator.errors)}`)
    }
}
