import { compileFilter, type JsonFilter, sql } from "@truto/sqlite-builder"
...
function assertKnownColumns(names: string[], known: Set<string>, label: string): void {
...
function assertBaseMetricColumns(names: string[], baseKnown: Set<string>, label: string): void {
    for (const name of names) {
        assertSafeIdentifier(name, "column")
...
function qualifyColumnExpression(columnName: string, outputSql: Map<string, string>): string {
...
function qualifyFilterText(filterText: string, outputSql: Map<string, string>): string {
    let text = filterText
...
function seedBaseOutputSql(baseTable: TableRow): {
    baseTableName: string
    baseColumns: ColumnSpec[]
    ...
    for (const column of baseCompiled) {
        outputSql.set(column.slug, `${basePrefix}.${quoteIdentExport(column.slug)}`)
    }

    const primaryKeys = primaryKeySlugs(baseDefinition)
    if (primaryKeys.length === 1) {
        outputSql.set("id", `${basePrefix}.${quoteIdentExport(primaryKeys[0]!)}`)
    } else {
...
export function resolveJoinGraph(baseTable: TableRow, joins: RefJoinSpec[]): ResolvedJoinGraph {
    ...
    joins.forEach((joinSpec, joinIndex) => {
        ...
        const refColumn = sourceDefinition.columns.find((column) => column.slug === refSlug)
        if (!refColumn) {
            throw new Error(
        ...
        for (const [sourceColumn, outputColumn] of Object.entries(joinSpec.select)) {
            ...
            seeded.outputSql.set(outputSlug, `${joinPrefix}.${quoteIdentExport(sourceColumnSlug)}`)
            joinOutputs.push({
...
function appendOrderClause(
    ...
    for (const orderSpec of order) {
        ...
        const direction =
            orderSpec.direction === "desc" ? "DESC" : orderSpec.direction === "asc" ? "ASC" : "ASC"
...
export function compileSelectQuery(definition: QueryDefinition): {
    ...
    if (joins.length === 0) {
        ...
        const query = sql`SELECT ${selectSql} FROM ${fromIdent}`
    ...
    text = appendOrderClause(text, definition.order, resolved.known, resolved.outputSql, true)
...
export function compileCountQuery(
    ...
    if (joins.length === 0) {
        const fromIdent = sql.ident(table.name)
        const query = sql`SELECT COUNT(*) AS total FROM ${fromIdent}`
...
export function compileAggregateQuery(
    ...
    const resolved = joins.length > 0 ? resolveJoinGraph(table, joins) : null
    ...
    for (const metric of definition.metrics) {
        ...
        selectParts.push(`${fn}(${metricColumn}) AS ${quoteIdentExport(alias)}`)
    }
    ...
    if (groupBy.length > 0) {
        const groupExpressions = groupBy.map((name) =>
            useQualifiedColumns ? qualifyColumnExpression(name, outputSql) : quoteIdentExport(name)
        )
