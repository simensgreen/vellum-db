...
const MIGRATION_ROW_SELECT =
  "SELECT id, hash, name, migration_json, applied_at FROM _migrations";
...
export function getMigrationById(
  ...
  return (
    database
      .query<MigrationRow, [number]>(`${MIGRATION_ROW_SELECT} WHERE id = ?`)
      .get(id) ?? null
  );
}
...
export function listMigrations(input: {
  limit?: number;
  offset?: number;
} = {}): {
  migrations: Array<{
  ...
  const rows = database
    .query<
      Pick<MigrationRow, "id" | "hash" | "name" | "applied_at">,
      [number, number]
    >(
