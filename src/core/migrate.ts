...
import { MigrationFileSchema, type MigrationFile } from "../api/schemas/migrate.ts";
...
function filterAlterOperation(
  ...
  const filteredDrop = (operation.drop ?? []).filter((dropSlug) =>
    existingSlugs.has(dropSlug),
  );
  const scopeProvided = Object.prototype.hasOwnProperty.call(operation, "scope");
  if (
    ...
    switch (operation.kind) {
      ...
      case "alter": {
        ...
        alterUserTable({
          ...
          add: filtered.add,
          drop: filtered.drop,
          ...(Object.prototype.hasOwnProperty.call(filtered, "scope")
...
function applyMigrationContent(input: {
  ...
  database.run("BEGIN");
  try {
    const operations = executeMigrationPlan(
      compileMigrationPlan(input.migration),
    );
...
export function listMigrationsView(input: {
  limit?: number;
  offset?: number;
}) {
