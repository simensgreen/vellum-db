...
function writeRowsToFile(
  ...
  writeFileSync(
    path,
    serializeRowsToBuffer(mode, rows, headers, sheetName),
  );
}
...
function fetchAllDecodedRows(table: TableRow): Record<string, unknown>[] {
  ...
  const selectList = columns
    .map((column) => quoteIdentExport(column.name))
    .join(", ");
  const orderBy = primaryKeySlugsList
    .map((slug) => quoteIdentExport(slug))
    .join(", ");
  const rawRows = getDatabase()
    .query(
      `SELECT ${selectList} FROM ${quoteIdentExport(table.name)} ORDER BY ${orderBy}`,
    )
...
export function dumpTableToBuffer(input: {
  table: string;
  mode: IoMode;
}): {
  table: string;
