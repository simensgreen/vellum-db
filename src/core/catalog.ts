...
import { invalidateSchemaCache, type JsonSchemaObject } from "../schema-validate.ts"
...
export function alterUserTable(input: {
    ...
    const dropSet = new Set(dropSlugs.map((slug) => assertSafeIdentifier(slug, "column")))
    ...
    const nextColumns = definition.columns.filter((column) => !dropSet.has(column.slug))
    ...
    const schemaJson = JSON.stringify(compileRowJsonSchema(nextDefinition, { knownTables }))
    const database = getDatabase()
    const timestamp = nowIso()
    ...
    try {
        ...
        if (dropSlugs.length > 0) {
            ...
            database.run(`ALTER TABLE ${quoteIdent(tempName)} RENAME TO ${quoteIdent(table.name)}`)
        }
...
function propertyTypeName(schema: JsonSchemaObject, columnName: string): string | undefined {
