...
function snapshotFromRow(row: StatsRow): Omit<StatsBucket, "start" | "inserts" | "updates" | "deletions" | "reads" | "total"> {
  return {
...
function buildBucketStarts(
  ...
  const maxLookbackDays = Math.min(
    retentionDays,
    bucketLimit * daysPerBucket(granularity),
  );
...
export function getDatabaseStats(input: {
  granularity?: StatsGranularity;
  limit?: number;
} = {}): DatabaseStats {
  ...
  const bucketStarts = buildBucketStarts(
    today,
    granularity,
    bucketLimit,
    retentionDays,
  ...
  const earliestDay =
    bucketStarts.length > 0 ? isoToEpochDay(bucketStarts[0]!) : today;
